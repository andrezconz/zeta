...
export function AddHoldingForm({ brokerAccountId }: { brokerAccountId: string }) {
  const [type, setType] = useState<AssetClass>("Acciones");
      ...
      <form action={createHoldingAction} className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <div>
          ...
          <input
            name="asset"
            required
            placeholder={isCdt ? "Ej. CDT Bancolombia" : "Ej. Apple Inc."}
            className={fieldClass}
          />
        </div>
        <div>
          <label className={labelClass}>Ticker</label>
          <input name="ticker" required placeholder={isCdt ? "CDT-BCO" : "AAPL"} className={fieldClass} />
        </div>
        <div>
        ...
        {isFondo && (
          <>
            <div>
              <label className={labelClass}>Monto invertido</label>
              <input name="investedAmount" type="number" step="any" required className={fieldClass} />
            </div>
            <div>
              <label className={labelClass}>Fecha de inversión</label>
              <input name="startDate" type="date" required className={fieldClass} />
            </div>
            <div>
              <label className={labelClass}>Rentabilidad mes (%)</label>
              <input name="monthlyReturn" type="number" step="any" defaultValue={0} className={fieldClass} />
            </div>
            <div>
              <label className={labelClass}>Rentabilidad semestre (%)</label>
              <input name="semesterReturn" type="number" step="any" defaultValue={0} className={fieldClass} />
            </div>
            <div>
              <label className={labelClass}>Rentabilidad año (%)</label>
              <input name="annualReturn" type="number" step="any" defaultValue={0} className={fieldClass} />
            </div>
          </>
        )}
        ...
        {isCdt && (
          <>
            ...
            <div>
              ...
              <input name="effectiveAnnualRate" type="number" step="any" required className={fieldClass} />
            </div>
          </>
        )}
